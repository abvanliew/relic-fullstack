...
use super::{Bonus, ModifierClass, ModifierSet};
...
impl ModifierClass {
  pub fn display_with_value(&self, bonus: &Bonus<i32>) -> String {
    ...
    match self {
      ...
      ModifierClass::CapabilitySpecialization => format!("+{value} Capability Specialization"),
      ModifierClass::DefenseRank => format!("+{value} Defense Ranks"),
      ModifierClass::DefenseSpecialization => format!("+{value} Defense Specialization"),
      ModifierClass::ExpertiseRank => format!("+{value} Expertise Ranks"),
      ModifierClass::ExpertiseSpecialization => format!("+{value} Expertise Specialization"),
      ModifierClass::InitiatePathRequired => format!("{value} Path Minimum"),
      ModifierClass::InitiatePathMax => format!("{value} Path Maximum"),
      ModifierClass::InitiatePathOptional => format!("{value} Path Optional"),
      ModifierClass::JourneymanPathRequired => format!("{value} Journeyman Path Minimum"),
      ModifierClass::JourneymanPathOptional => format!("{value} Journeyman Path Maximum"),
      ModifierClass::MasterPathRequired => format!("{value} Master Path Minimum"),
      ModifierClass::MasterPathOptional => format!("{value} Master Path Maximum"),
      ModifierClass::DevelopmentPoints => format!("{value} Growth Ranks"),
      ...
      ModifierClass::ManaPool => format!("+{value} Mana Pool"),
      ModifierClass::ManaPoolAll => format!("+{value} all Mana Pools"),
...
impl Display for ModifierSet {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    let display_bonuses: Vec<String> = bonuses
      .iter()
      .map(|(class, bonus)| class.display_with_value(bonus))
      .collect();
