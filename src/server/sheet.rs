...
pub async fn list_character_sheets() -> Result<Vec<CharacterSheet>, ServerFnError> {
  ...
  let characters_collection: Collection<CharacterSheet> =
    client.database("relic").collection("creatures");
  ...
  while let Some(result) = results.next().await {
    let Ok(character) = result else {
      tracing::error!("Unable to load character {:?}", result);
      continue;
    };
...
pub async fn get_chracter_sheet(id: String) -> Result<CharacterSheet, ServerFnError> {
  let client = Client::with_uri_str("mongodb://localhost:27017").await?;
  let sheets_collection: Collection<CharacterSheet> =
    client.database("relic").collection("creatures");
  ...
  return Err(ServerFnError::ServerError(format!(
    "Unable to find chracter sheet with id: {}",
    &id
  )));
}
