...
pub fn CharacterProgression() -> Element {
  ...
  let mut all_path_ids = path_map_cache
    .into_vec()
    .iter()
  ...
  for skill in &selectable_skills {
    ...
    if skill_mask != 0 {
      let constraint_set = constraint_combos
        .entry(skill_mask)
        .or_insert(ConstraintSet {
          required_weight,
          filters,
          ..Default::default()
        });
  ...
  for (index, constraint) in constraints.iter_mut().enumerate() {
    ...
    for (combo_mask, combo_constraint) in constraint_combos.iter_mut() {
      if constraint_mask & combo_mask == 0 {
        continue;
      }
      let constraint_net = (combo_constraint.selected_weight - combo_constraint.required_weight
  ...
  for (combo_mask, combo_constraint) in constraint_combos.iter_mut() {
    ...
    combo_constraint.leeway = (combo_constraint.required_weight
  ...
  for (skill_id, mask) in skill_constraints {
    let Some(constraint_set) = constraint_combos.get(&mask) else {
      continue;
    };
    skill_selection
      .leeway
      .insert(skill_id, constraint_set.leeway);
  }

  let mut core_constraints: Vec<String> = Vec::new();
  for (index, constraint) in constraints.iter().enumerate() {
    let mask = 1 << index;
    let Some(combo_constraint) = constraint_combos.get(&mask) else {
      continue;
    };
    ...
    core_constraints.push(format!(
      "{}/{} {} from {}",
      ranks, max_rank, skill_name, filter_name
    ));
  }
