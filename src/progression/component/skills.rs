...
pub struct SkillSelections {
  ...
  pub skill_rank_signal: Signal<HashMap<String, i32>>,
...
impl Default for SkillSelections {
  fn default() -> Self {
    Self {
      ...
      skill_rank_signal: use_signal(|| HashMap::new()),
...
pub fn SkillSelector(
  ...
  // let instance_result = skill.to_instance_result();
  let rank: Signal<i32> = use_signal(|| 0);
  rsx! {
    div {
      ...
      onclick: move |_| {
        match selected {
          true => {
            // skill_selections.remove(&id);
          },
          false => {
            // if let Some( instance ) = &instance_result {
            //   skill_selections.insert( id.clone(), instance.clone() );
            // }
          }
        }
        // skill_selections_signal.set(skill_selections.clone());
      },
      SkillCard {
        skill,
        display: SkillTermDisplay::Embeded,
        ranked_input: Some( ( rank, 10 ) ),
      }
    }
  }
}
